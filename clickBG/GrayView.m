...

{
    ...
    if(self = [super initWithFrame: frame]){
        ...
        fadeAlpha= (fAlpha < 0 ? 0.50 : fAlpha);
...
appendBezierPathWithArcWithCenter:NSMakePoint(wide-roundingSize,high-roundingSize)
    ...
    if(pointCorner==0){
        ...
        [fadePath lineToPoint: NSMakePoint(roundingSize,high)];
    ...
    if(pointCorner==2){
        ...
        [fadePath lineToPoint: NSMakePoint(ox,taily)];
        [fadePath lineToPoint: NSMakePoint(roundingSize,oy)];
    }else{
    [fadePath appendBezierPathWithArcWithCenter:NSMakePoint(roundingSize,oy+roundingSize)
                                    radius: roundingSize
                                startAngle:180.0
                                    endAngle:270.0];
    }
...
- (NSRect) preferredFrame
{
    ...
    return NSMakeRect([self frame].origin.x,[self frame].origin.y,((roundingSize - insetSize)*2) + textSize.width,((roundingSize - insetSize)*2)+textSize.height);
...
- (void)drawRect:(NSRect)rect
{
    ...
    NSPoint inside=NSMakePoint(rect.origin.x+(roundingSize - insetSize),rect.origin.y+(roundingSize - insetSize) + (pointCorner==2||pointCorner==3 ? tailLen: 0));
    if(dirty){
        [textArea setSize: rect.size];
        [textArea lockFocus];
        [[NSColor clearColor] set];
